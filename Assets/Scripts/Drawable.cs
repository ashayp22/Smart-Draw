...
namespace FreeDraw
{
    ...
    public class Drawable : MonoBehaviour
    {
        ...
        private bool colored = false; //error comes up when you don't draw anything, and reference cur_colors - this fixes it

        public AudioSource pencilAudio;
        public AudioSource eraserAudio;
...
//////////////////////////////////////////////////////////////////////////////
        ...
        void Update()
        {
            ...
            {
                cur_colors[0] = new Color32(255, 0, 0, 255);
                cur_colors[175980] = new Color32(0, 255, 0, 255);

                cur_colors[419] = new Color32(0, 0, 255, 255);
        ...
        public void clearCanvas()
        {
            ...
            {
                cur_colors[i] = new Color32(255, 255, 255, 255);
            }
            ApplyMarkedPixelChanges();
        }

        //returns the screen in pixels, 28 * 28
        public List<double> getPixels()
        {
            //format the screen into data


            //get full size 420 * 420
            double[,] screen = new double[420, 420];

            for (int i = 0; i < 420; i++)
            {
                for (int j = 0; j < 420; j++)
                {
                    //check to see if its a white pixel
                    if (cur_colors[i * 420 + j].r == 255 && cur_colors[i * 420 + j].g == 255 && cur_colors[i * 420 + j].b == 255)
                    {
                        screen[j, 419 - i] = 0; //white pixel
                    }
                    else
                    {
                        screen[j, 419 - i] = 255; //colored pixel
                    }
                }
            }
            ...
            //first format the image since it is too big

            int multiplier = window_size / 28;

            for (int i = 0; i < 420; i++)
            {
                for (int j = 0; j < 420; j++)
                {
                    features[j / multiplier, i / multiplier] += screen[i, j];
                }
            }
            ...
            List<double> formatted = new List<double>();
            ...
            for (int i = 0; i < 28; i++)
            {
                ...
                {
                    //normalize
                    double d = features[i, j];
                    d /= Mathf.Pow(multiplier, 2);
                    d /= 255;
        ...
        {
            foreach (Color32 c in cur_colors)
            {
                if (c.r != 255 || c.g != 255 || c.b != 255) //is not white
                {
                    return false;
                }
            }
            return true; //all are white
        }
